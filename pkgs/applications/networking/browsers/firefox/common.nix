...
buildStdenv.mkDerivation {
  ...
  '' + lib.optionalString stdenv.hostPlatform.isDarwin ''
    # build system doesn't honor BINDGEN_EXTRA_CLANG_ARGS at all; enforce access to SDK
    substituteInPlace config/makefiles/rust.mk --replace-fail \
      'export BINDGEN_EXTRA_CLANG_ARGS:=$(filter --target=%,$(BINDGEN_SYSTEM_FLAGS))' \
      'export BINDGEN_EXTRA_CLANG_ARGS:=$(filter --target=%,$(BINDGEN_SYSTEM_FLAGS)) -F${apple-sdk_14.sdkroot}/System/Library/Frameworks'
  ''
  + extraPostPatch;
  ...
  postInstall = lib.optionalString stdenv.hostPlatform.isDarwin ''
    ...
    resourceDir=$(ls -d $out/Applications/*.app/Contents/Resources)
  ...
  installCheckPhase = lib.optionalString buildStdenv.hostPlatform.isDarwin ''
    bindir=$(ls -d $out/Applications/*.app/Contents/MacOS)
